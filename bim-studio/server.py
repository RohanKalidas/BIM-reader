...
def upload_ifc():
    ...
    try:
        result=subprocess.run(["python3","run.py",filepath],capture_output=True,text=True,timeout=300,cwd=REPO_DIR)
        print("STDOUT:",result.stdout[-500:])
        if result.returncode!=0: return jsonify({"error":"Pipeline failed","log":result.stderr}),500
        project_id=None
        ...
        if not project_id: return jsonify({"error":"Could not determine project_id"}),500
        ...
        cur.close(); conn.close()
        return jsonify({"status":"done","project_id":project_id,"aps":aps_result,"stats":{"components":total,"relationships":rels}})
...
def reconstruct():
    ...
    try:
        result=subprocess.run(["python3","reconstruct.py",str(project_id)],capture_output=True,text=True,timeout=120,cwd=REPO_DIR)
...
LIBRARY COMPONENTS
Each library component has an "id". When placing furniture, fixtures, or any item matching a library component, set "library_component_id" to that id — the system transplants real geometry from the source IFC file. If no match exists, generate parametrically with realistic dimensions.
...
SPEC FORMAT
The <building_spec> block is valid JSON, completely hidden from the user. Include ALL systems. A complete residential building should have 40-80+ components across all trades. A commercial building 80-200+.
...
def generate_stream():
    ...
    def generate():
        ...
        with client.messages.stream(model="claude-sonnet-4-20250514",max_tokens=16000,system=GENERATE_SYSTEM_PROMPT,messages=messages) as stream:
...
def compose():
    ...
    def generate():
        full_text=""
        with client.messages.stream(model="claude-sonnet-4-20250514",max_tokens=2000,system=COMPOSE_SYSTEM_PROMPT,messages=messages) as stream:
            for text in stream.text_stream:
